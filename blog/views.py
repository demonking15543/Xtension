...
def about_author(request, username):
    ...
    author_details = User.objects.get(username=username)
    published_post_by_user = Post.objects.filter(author=author_details.id)
    abouts = Profile.objects.get(id=author_details.id)
    following = author_details.followers.all()
    ...
    context = {
        ...
        'abouts': abouts,
        'post': published_post_by_user.count(),
